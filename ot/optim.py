...
def cg(a, b, M, reg, f, df, G0=None, numItermax=200,
    ...
    **kwargs : dict
    ...
    while loop:
        ...
        # Gc_ = np.argmin(Mi, axis=1)
        # Gc = np.zeros(Mi.shape)
        # Gc[np.arange(len(Gc)), Gc_] = 1
        # Gc = Gc / np.sum(Gc)
        deltaG = Gc - G
        ...
        # print("previous loss", np.sum(G * Mi))
        G = G + alpha * deltaG
        # print("new loss", np.sum(G * Mi))
        # print("only Gc", np.sum(Gc * Mi))
        # test convergence
        if it >= numItermax:
            loop = 0
